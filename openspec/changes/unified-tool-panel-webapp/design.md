...
- **Reads via `getActiveSpreadsheet()` (narrow scope).** A `getSpreadsheet_()` helper returns the active spreadsheet. The Web App iteration used `openById` (a `doGet` has no active spreadsheet), but `openById` requires the broad all-spreadsheets scope — which the user had not authorized, producing "Authorization is required" from the menu. The modal runs in-sheet, so `getActiveSpreadsheet()` works and needs only the same narrow container scope the original sidebars already used — no new consent.
